...
public final class ApiKeyNewOneView extends BaseHtmlView {
  ...
  public Content render(Request request, DynamicForm form, ImmutableSet<String> programNames) {
    ...
    ContainerTag formTag =
        form()
            ...
            .with(
                ...
                p(
                    "Specify a subnet using CIDR notation that is allowed to call the API. A"
                        + " single IP address can be specified with a mask of /32. For example,"
                        + " \"8.8.8.8/32\" allows only the IP 8.8.8.8 to use the API key. All IP"
                        + " addresses can be allowed, but this is strongly discouraged for"
                        + " security reasons because it would allow any computer connected to the"
                        + " internet to use the key if they obtain it. All IP addresses can be"
                        + " allowed with a value of 0.0.0.0/0."),
    ...
    HtmlBundle htmlBundle =
        layout.getBundle().setTitle(title).addMainContent(contentDiv);
